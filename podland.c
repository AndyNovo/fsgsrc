...
#define VERSION 445 //id for the validation
...
int portalLocation(int64_t seed, int villageMode, int* px, int* pz){
  ...
  if (villageMode == 1){
    if (!valid2 || p2.x >= 112 || p2.z >= 112){// || p2.x >= 144 || p2.z >= 144){
      return 0;
    }
    *px = p2.x;
    *pz = p2.z;
...
int portalTypeNormal(int64_t seed){
  ...
  if (bigOrSmall < .05){
    return  10 + (rawPortalType % 3);//all three big ones have enough lava and we're not underground
  }
  portalType = rawPortalType % 10;
  return portalType;
  if (portalType == 0 || portalType == 2 || portalType == 3 || portalType == 4 || portalType == 5 || portalType == 8){
...
int portalLoot(int64_t lower48, int px, int pz, int pType){
  ...
  int obiT[13] = {2, 4, 4, 3, 5,    1, 3, 3, 2, 7,   5, 5, 5};
  int obicutoff = 4;
  if (pType >= 0 && pType <= 12){
  ...
  if (lootb > 0 && obic >= obicutoff && fireb > 0 && axeeq > 0){
    fitness += 100;
  }
...
int villageBiome(int64_t seed, LayerStack* gp){
  ...
  int biome = getBiomeAtPos(gp, p);
  if (biome % 128 == snowy_tundra || biome % 128 == desert){
    return 0;
  }
...
int main () {
    ...
    char descriptions[5][65] = {"(Axe, Looting, Lighter, Obi Portal, Village)", "(Village Only)", "(Shipwreck Only)", "(Jungle Only)", ""};
    printf("FSG v1.2.5p0d %s\n", descriptions[FILTER_TYPE]);
    ...
    for(ii=0; is_done < 1; ii++){
      ...
      if (has_lower < 1){
        ...
        result = valid_structures_and_types2(lower48, &fortressQuadrant, filterStyle, &px3, &pz3);
